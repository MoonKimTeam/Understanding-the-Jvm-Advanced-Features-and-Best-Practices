...
- 톰캣, 제티, 웹로직, 웹스피어 등 주류 자바 웹 서버가 둘 이상의 클래스 로더 혼용하는 이유
    - 똑같은 서버에 배포하더라도 자바 클래스 라이브러리를 웹 애플리케이션 별로 격리하는 게 좋음
    - 똑같은 서버에 배포된 둘 이상의 웹 애플리케이션이사용하는 자바 클래스 라이브러리는 서로 공유 가능
    - 서버는 보안을 최대한 유지해서 배포된 웹 애플리케이션들의 영향을 받지 않아야
    - JSP 애플리케이션을 지원하는 웹 서버는 대부분 핫 스와프를 지원하며, 가상 머신에서 실행되기 전 자바 클래스 파일로 컴파일됨
        - 하지만, 서드 파티 클래스 라이브러리나 자바로 작성된 코드보다 런타임에 수정될 가능성이 훨씬 큼
...
- 클라이언트는 '임시 코드'의 실행 결과를 받아 볼 수 있다. 실행 결과에는 프로그 램에서 출력한 정보와 발생한 예외가 포함될 수 있다.
